...
def run_epoch(epoch_id, model, data_iter, loss_fn, device, mylog, optimizer=None, print_every=20, is_training=True):
    if is_training:
        model.train()
    else:
        model.eval()
    ...
    for i, data in enumerate(data_iter):
        ...
        batch_errs, batch_tokens = model.compute_wer(index.transpose(0, 1).cpu().numpy(), input_sizes.cpu().numpy(),
    ...
    print(("Epoch %d %s done, total_loss: %.4f, total_wer: %.4f" % (
    epoch_id, training, average_loss, total_errs / total_tokens)), file=mylog)
...
def main(conf):
    ...
    for k, v in conf.items():
        setattr(opts, k, v)
        print('{:50}:{}'.format(k, v))
        print(('{:50}:{}'.format(k, v)), file=mylog)

    device = torch.device('cuda:0') if opts.use_gpu else torch.device('cpu')
    # device=torch.device('cpu')

    # model_path = os.path.join(opts.checkpoint_dir, opts.exp_name, 'ctc_best_model_old.pkl')
    ...
    print(("Number of parameters %d" % num_params), file=mylog)

    for idx, m in enumerate(model.children()):
        print(idx, m)
        print(idx, m, file=mylog)
    ...
    while not stop_train:
        ...
        print("Start training epoch: %d, learning_rate: %.5f" % (count, learning_rate), file=mylog)

        train_acc, loss = run_epoch(count, model, train_loader, loss_fn, device, mylog, optimizer=optimizer,
                                    print_every=opts.verbose_step, is_training=True)
        loss_results.append(loss)
        acc, dev_loss = run_epoch(count, model, dev_loader, loss_fn, device, mylog, optimizer=None,
                                  print_every=opts.verbose_step, is_training=False)
        print("loss on dev set is %.4f" % dev_loss)
        print("loss on dev set is %.4f" % dev_loss, file=mylog)
        ...
        print("adjust_rate_count:" + str(adjust_rate_count), file=mylog)
        print('adjust_time:' + str(adjust_time))
        print('adjust_time:' + str(adjust_time), file=mylog)
        ...
        print("epoch %d done, cv acc is: %.4f, time_used: %.4f minutes" % (count, acc, time_used), file=mylog)
        print('loss_best:', loss_best)
        print('loss_best:', file=mylog)
    ...
    print("End training, best dev loss is: %.4f, acc is: %.4f" % (loss_best, acc_best), file=mylog)

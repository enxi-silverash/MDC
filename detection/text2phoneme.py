...
def text_to_phonemes(text: str, cmu_dict_path: str):
    ...
    for word in text.split(' '):
        ...
        if cmu_word != word:
            ...
            cmu_result.append(cmu_word.lower())
...
def text2phoneme_interface(str_text):
    cmu_dict = "./detection/cmudict.txt"
    print("如果路径出现问题，需要修改cmudict的路径")
    unfinished_phoneme = text_to_phonemes(str_text, cmu_dict)
    # unfinished_phoneme:{...} {...} {...}
    # print(unfinished_phoneme)
    b = re.split("{|}", unfinished_phoneme)
    # print("b:",b)
    for i in b:
        if i == ' ' or i == '':
            try:
    ...
    phoneme_str = ""
    for i in b:
        phoneme_str = phoneme_str.lower() + i.lower() + ' '
    phoneme_str = ''.join([i for i in phoneme_str if not i.isdigit()])
    phoneme_str.strip()
    print("phoneme_str:", phoneme_str)
    ...
    for i in range(len(b)):
        ...
        for j in range(len(z)):
            # z[j]=z[j].lower()
            q = z[j]
            z[j] = ''.join([k for k in q if not k.isdigit()]).lower()
        phonemes.append(z)

    # print("phonemes:",phonemes)
    # words_phonemes_match
    w_p_m = []

    for i in range(len(phonemes)):
        if len(phonemes) != len(words):
            print("The number of phonemes and words doesn't match")
            break
            ...
            # 加入单词和对应音素
            w_p.append(words[i])
            w_p.append(phonemes[i])
            print("w_p:", w_p)
            w_p_m.append(w_p)
            print("w_p_m:", w_p_m)
    # print("words_phonemes_match:",w_p_m)
    return words, phonemes, phoneme_str, w_p_m
